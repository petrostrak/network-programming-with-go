package pushingresourcestotheclient
...
import (
	...
	middleware "github.com/petrostrak/network-programming-with-go/07-Building-HTTP-Services/Middleware"
...
func run(addr, files, cert, pkey string) error {
	...
	mux.Handle("/static/", http.StripPrefix(
		"/static/", middleware.RestrictPrefix(".", http.FileServer(http.Dir(files)))))
